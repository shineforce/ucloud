...
function UCloud_PutFile($bucket, $key, $file)
{
    ...
    $host = $bucket . config('ucloud.UCLOUD_PROXY_SUFFIX');
    $path = $key;
...
{
    return $bucket . config('ucloud.UCLOUD_PROXY_SUFFIX') . "/" . rawurlencode($key);
}
...
function UCloud_MakePrivateUrl($bucket, $key, $expires = 0)
{
    ...
    $url = $public_url . "?UCloudPublicKey=" . rawurlencode(config('ucloud.UCLOUD_PUBLIC_KEY')) . "&Signature=" . rawurlencode($signature);
    if ('' != $expires) {
